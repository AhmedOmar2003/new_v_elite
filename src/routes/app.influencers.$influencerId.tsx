  ...
  return (
    <Panel>
      <PanelBody>
        ...
        <p className="mt-5">
          <ArrowLink to="/app/influencers">Back to creators</ArrowLink>
        </p>
      </PanelBody>
    </Panel>
  );
...
function InfluencerProfile() {
  ...
  */
  ...
  const facts: (readonly [string, ReactNode])[] = [
    ...
    ...(top ? ([["Avg. cost per published item", top.cost] as const] as const) : []),
  ];
    ...
    <>
      <PageHead
        crumbs={[
          ...
          { label: "Creators", to: "/app/influencers" },
          { label: creator.name },
        ]}
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-[1.6fr_1fr]">
        ...
        <div className="grid content-start gap-5">
          ...
          <Panel>
            <PanelHead
              title={`Similar creators in ${creator.categoryLabel}`}
              sub="Creators with a similar audience profile"
            />
            <PanelBody>
          ...
          <Panel>
            <PanelHead title="Latest check-ins" sub="Recorded by the branch scanner" />
            <PanelBody>
              {visits.length > 0 ? (
                <Feed
                  items={visits.map((v) => ({
                ...
                <p className="text-sm text-muted-foreground">
                  No check-ins were recorded for this creator during the selected reporting period.{" "}
                  <Link
                    to="/app/scanner"
...
function StoryRail({
  ...
  return (
    <Panel className="mt-6">
      <PanelHead
        title="Live stories"
        sub={`${frames.length} story frames are live now. Some expire within 24 hours.`}
        actions={
...
function ContentPanel({ posts, reels }: { posts: CreatorMedia[]; reels: CreatorMedia[] }) {
  ...
  return (
    <Panel>
      <PanelHead
        title="Recent campaign content"
        sub={`${posts.length} posts and ${reels.length} reels published for your campaigns.`}
        actions={
          <Segmented
            label="Content type"
            options={["Posts", "Reels"]}
      ...
      <PanelFoot>
        <span className="text-xs text-muted-foreground">
          Demo content shown for illustration only.
        </span>
      </PanelFoot>
    </Panel>
  );
...
function CoveragePanel({
  ...
  return (
    <Panel>
      ...
      {coverage.length === 0 ? (
        <PanelBody>
          <EmptyState
            ...
            body={`${name} isn’t on a live campaign in this demo workspace. Add them to a shortlist and their published content will appear here.`}
      ...
      <PanelFoot>
        <span className="text-xs text-muted-foreground">
          Delivery is measured against each campaign’s agreed deliverables.
        </span>
      </PanelFoot>
    </Panel>
  );
