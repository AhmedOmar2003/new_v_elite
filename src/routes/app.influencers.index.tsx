...
export const Route = createFileRoute("/app/influencers/")({
  head: () => ({ meta: [{ title: "Creators | ELITƎ workspace" }] }),
...
function Influencers() {
  ...
  function toggleSave(handle: string, name: string) {
    setSaved((prev) => {
      ...
      if (wasSaved) {
        next.delete(handle);
        demo(`Removed ${name} from saved creators`);
      } else {
        next.add(handle);
        demo(`Saved ${name}`, "This creator is ready to add to a shortlist.");
      }
      return next;
    });
  }
  ...
  return (
    <>
      <PageHead
        crumbs={[{ label: "Dashboard", to: "/app" }, { label: "Creators" }]}
        title="Creators"
        lede="Browse the ELITƎ creator network, save profiles, and build shortlists your team can use for upcoming campaigns."
        actions={
          <>
            ...
            <AppButton tone="primary" onClick={() => setShortlistModal(true)}>
              <Plus className="size-4" /> Create shortlist
            </AppButton>
          </>
        }
      />
                ...
                >
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label={
                        isSaved ? `Remove ${c.name} from saved creators` : `Save ${c.name}`
                      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-md">
          <div className="w-full max-w-md rounded-2xl border border-border bg-[color:var(--bg-raised)] p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <div>
                <h3 className="text-base font-bold">Create shortlist</h3>
                <p className="text-xs text-muted-foreground">
                  Organize saved creators for an upcoming campaign.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateShortlist} className="mt-4 space-y-4">
              ...
              <div className="rounded-lg border border-border bg-[color:var(--bg-sunken)] p-3 text-xs text-muted-foreground">
                <p className="font-semibold text-foreground">Saved creators ({saved.size})</p>
                <p className="mt-1">
                  All {saved.size} saved creators will be added to this shortlist.
                </p>
              </div>
